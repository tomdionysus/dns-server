...
class DNSPacket {
	...
	deserialize(des) {
		...
		this.flags = this._parseFlags(des.readUInt16())
	...
	serialize(ser) {
		...
		ser.writeUInt16(this._unParseFlags(this.flags))
	...
	_unParseFlags(flags) {
		var out = 0
		if(flags.response) 			 out |= 0b1000000000000000
		if(flags.authoriative) 		 out |= 0b0000010000000000
		if(flags.truncated) 		 out |= 0b0000001000000000
		if(flags.recursionDesired) 	 out |= 0b0000000100000000
		if(flags.recursionAvailable) out |= 0b0000000010000000
		out |= ((flags.opCode & 0b1111) << 11)
		out |= ((flags.responseCode & 0b1111))
		return out
	}
	...
	_readAnswerRR(des) {
		...
		var rrdes = new BinaryDeserializer({ buffer: out.rddata, parent: des })

		switch(out.qtype) {
			case DNSPacket.QType.A:
				out.rddata = this._readARR(rrdes)
				break
			case DNSPacket.QType.NS:
				out.rddata = this._readNSRR(rrdes)
				break
			case DNSPacket.QType.CNAME:
				out.rddata = this._readCNAMERR(rrdes)
				break
			case DNSPacket.QType.SOA:
				out.rddata = this._readSOARR(rrdes)
				break
			case DNSPacket.QType.PTR:
				out.rddata = this._readPTRRR(rrdes)
				break
			case DNSPacket.QType.MX:
				out.rddata = this._readMXRR(rrdes)
				break
			case DNSPacket.QType.TXT:
				out.rddata = this._readTXTRR(rrdes)
				break
			case DNSPacket.QType.SRV:
				out.rddata = this._readSRVRR(rrdes)
				break
			case DNSPacket.QType.AAAA:
				out.rddata = this._readAAAARR(rrdes)
				break
		}
	...
	_writeAnswerRR(ser, answer) {
		...
		var rrser = new BinarySerializer({ parent: ser })

		switch(answer.qtype) {
			case DNSPacket.QType.A:
				this._writeARR(rrser, rddata)
				break
			case DNSPacket.QType.NS:
				this._writeNSR(rrser, rddata)
				break
			case DNSPacket.QType.CNAME:
				this._writeCNAMER(rrser, rddata)
				break
			case DNSPacket.QType.SOA:
				this._writeSOAR(rrser, rddata)
				break
			case DNSPacket.QType.PTR:
				this._writePTRR(rrser, rddata)
				break
			case DNSPacket.QType.MX:
				this._writeMXR(rrser, rddata)
				break
			case DNSPacket.QType.TXT:
				this._writeTXTR(rrser, rddata)
				break
			case DNSPacket.QType.SRV:
				this._writeSRVR(rrser, rddata)
				break
			case DNSPacket.QType.AAAA:
				this._writeAAAAR(rrser, rddata)
				break
		}
	...
	_writeDomain(ser, domain = '') {
		...
		for(var i in out) {
			...
			var dompart = out.slice(i).join('.'), abso = ser.getAbsoluteOffset()
