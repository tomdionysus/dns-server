...
class DNSServer {
  ...
  _onMessage (msg, rinfo) {
    ...
    if (pkt.flags.response === false && pkt.question.length > 0) {
      ...
      this._getRRs(question.qname, DNSPacket.QType[question.qtype], (err, rrs) => { this._onGetRRs(err, rrs, pkt, rinfo) })
    } else {
      // Send FormErr
      this._sendFormatError(pkt, rinfo)
    }
  }

  _onGetRRs(err, rrs, pkt, rinfo) {
    // If DB Error, SvrFail
    if (err) return this._sendServerError(pkt, rinfo)
